...
public class SaleController {
    ...
    public String outStock(HttpServletRequest request, Model model) {
        ...
        try {
            ...
            if ("未发货".equals(flag)) {
                List<SaleItem> saleItems = saleService.findSaleOrderByOrderId(orderId);
                for (SaleItem item : saleItems) {
                    //修改
                    stockService.updateStockCountAfterSale(item.getName(), item.getCount());
                }
